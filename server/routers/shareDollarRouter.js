...
router.get('/api/private/sharedollar/teams/', async (req, res) => {
    ...
    for (const userTeam of userTeams) {
        const team = await db.get('SELECT * FROM share_dollar_teams WHERE id = ?', [userTeam.team_id])
        teams.push(team)
    }
...
router.post('/api/private/sharedollar/teams', async (req, res) => {
    ...
    const teamAlreadyExists = await db.get('SELECT * FROM share_dollar_teams WHERE LOWER(team_name) = ?', [teamName.toLowerCase()])
...
router.post('/api/private/sharedollar/teams/join', async (req, res) => {
    ...
    const team = await db.get('SELECT * FROM share_dollar_teams WHERE id = ?', [Number(teamId)])
    if (!team) {
        return res.status(404).send({
    ...
    const isAlreadyInTeam = await db.get('SELECT * FROM share_dollar_teams_users WHERE team_id = ? AND user_id = ?', [Number(teamId), userId])
    if (isAlreadyInTeam) {
        return res.status(400).send({
    ...
    const invitation = await db.get('SELECT * FROM share_dollar_teams_invites WHERE team_id = ? AND user_id = ?', [Number(teamId), userId])
    if (!invitation) {
        return res.status(400).send({
    ...
    return res.status(200).send({
        message: `Du er nu en del af ${team.team_name}`,
        status: 200
    });
});
...
router.delete('/api/private/sharedollar/teams/invite/:id', async (req, res) => {
    const invitation = await db.get('SELECT * FROM share_dollar_teams_invites WHERE id = ?', [Number(req.params.id)])
    if (!invitation) {
        return res.status(404).send({
...
router.get('/api/private/sharedollar/teams/invite', async (req, res) => {
    ...
    for (const invitation of invitationsFromDb) {
        const team = await db.get('SELECT * FROM share_dollar_teams WHERE id = ?', [invitation.team_id])
        const user = await db.get('SELECT * FROM users WHERE id = ?', [invitation.user_id])
...
router.post('/api/private/sharedollar/teams/invite', async (req, res) => {
    ...
    if (!email || !team_id) {
        return res.status(400).send({
            message: "Email mangler eller team mangler",
            status: 400
        });
    };
    const isOwnerOfTeam = await db.get('SELECT * FROM share_dollar_teams WHERE team_creator_id = ? AND id = ?', [req.session.user.id, Number(team_id)])
    if (!isOwnerOfTeam) {
        return res.status(400).send({
            message: "Du er ikke ejer af teamet",
            status: 400
    ...
    const userToInvite = await db.get('SELECT * FROM users WHERE LOWER(email) = ? AND verified = 1', [email.toLowerCase()])
    if (!userToInvite) {
        return res.status(404).send({
    ...
    const userAlreadyInTeam = await db.get('SELECT * FROM share_dollar_teams_users WHERE user_id = ? AND team_id = ?', [userToInvite.id, Number(team_id)])
    ...
    const userAlreadyInvited = await db.get('SELECT * FROM share_dollar_teams_invites WHERE user_id = ? AND team_id = ?', [userToInvite.id, Number(team_id)])
...
router.get('/api/private/sharedollar/teams/:id', async (req, res) => {
    const foundTeam = await db.get('SELECT * FROM share_dollar_teams WHERE id = ?', [Number(req.params.id)])
    if (!foundTeam) {
        return res.status(404).send({
            message: "Team ikke fundet",
            status: 404
    ...
    const isApartOfTeam = await db.get('SELECT * FROM share_dollar_teams_users WHERE user_id = ? AND team_id = ?', [req.session.user.id, Number(req.params.id)])
    if (!isApartOfTeam) {
        return res.status(400).send({
    ...
    const teamUsers = await db.all('SELECT * FROM share_dollar_teams_users WHERE team_id = ?', [Number(req.params.id)])
    const teamCreator = await db.get('SELECT * FROM users WHERE id = ?', [foundTeam.team_creator_id])
    ...
    const teamMembers = []
    for (const teamUser of teamUsers) {
        const user = await db.get('SELECT * FROM users WHERE id = ?', [teamUser.user_id])
        const { password, verified, verification_code, token, token_expiration, phone, verification_code_expiration, ...userWithoutPassword } = user;
...
router.get('/api/private/sharedollar/teams/:id/messages', async (req, res) => {
    ...
    for (const rawMessage of rawMessages) {
        const user = await db.get('SELECT * FROM users WHERE id = ?', [rawMessage.user_id])
        messages.push({
...
router.post('/api/private/sharedollar/teams/:id/messages', async (req, res) => {
    ...
    await db.run('INSERT INTO share_dollar_teams_messages (team_id, user_id, content, date) VALUES (?, ?, ?, ?)', [Number(req.params.id), req.session.user.id, message, new Date().toLocaleString("da-DK")])
    const sentMessage = {
...
router.delete('/api/private/sharedollar/teams/:id/members/:memberId', async (req, res) => {
    ...
    const isApartOfTeamMember = await db.get('SELECT * FROM share_dollar_teams_users WHERE user_id = ? AND team_id = ?', [Number(req.params.memberId), Number(req.params.id)])
    ...
    const userToRemove = await db.get('SELECT * FROM users WHERE id = ?', [Number(req.params.memberId)])
...
router.post('/api/private/sharedollar/teams/:id/requests', async (req, res) => {
    ...
    for (const request of requests) {
        const isReceiverApartOfTeam = await db.get('SELECT * FROM share_dollar_teams_users WHERE user_id = ? AND team_id = ?', [Number(request.memberId), Number(req.params.id)])
    ...
    const dbRequest = await db.run('INSERT INTO share_dollar_teams_money_requests (requestor_id, team_id, total_amount, paid, date) VALUES (?, ?, ?, ?, ?)', [req.session.user.id, Number(req.params.id), totalAmount, false, new Date().toLocaleString("da-DK")])
    for (const request of requests) {
    ...
    if (requests.length > 0) {
        for (const request of requests) {
            ...
            for (const requestUser of requestUsers) {
                const user = await db.get('SELECT * FROM users WHERE id = ?', [requestUser.receiver_id]);
...
router.patch('/api/private/sharedollar/requests/recieved/:id/pay', async (req, res) => {
    const request = await db.get('SELECT * FROM share_dollar_teams_money_requests_users WHERE id = ?', [Number(req.params.id)])
    if (!request) {
        return res.status(404).send({
