<script>
	...
	async function calculateDrivingDeduction() {
		startLoading(calculateDeductionButtonElement);
		...
		if (response.status === 200) {
			...
			stopLoading(calculateDeductionButtonElement);
		} else {
	...
	async function getDistance() {
		startLoading(calculateDistanceButtonElement);
		...
		if (response.status === 200) {
            ...
            stopLoading(calculateDistanceButtonElement);
		} else {
			toastr.error(data.message);
            stopLoading(calculateDistanceButtonElement);
		}
...
<main class="container">
	<hgroup>
		...
		<h3 class="top-m">
			Går du og tænker, "Hvad mon min kørselsfrdag er?" eller søger du efter et
			værktøj der kan hjælpe dig med at se din kørselsfrdag, så du kan få penge
			tilbage ? Så er dette det helt rigtige værktøj til dig.
		</h3>
	</hgroup>
	<details>
		...
		<form
			class="address-form"
			on:submit|preventDefault={getDistance}
		>
			<hgroup>
				<h2>Arbejdsadresse</h2>
				<h3>
					hvis du ikke kender den præcise kilometer afstand fra dit hjem til din
					arbejdsplads, kan du søge efter din arbejdsplads addresse herunder og
					få den præcise afstand.
				</h3>
			</hgroup>
			...
			{/if}
			<button
				bind:this={calculateDistanceButtonElement}
				type="submit"
			>
				Beregn afstand
			</button>
	...
	{#if drivingDeductionData.deductionTotal}
		<div id="driving-deduction-result">
			...
			<h3 class="top-m h2-h3">
				Dit kørselsfradrag er beregnet ud fra de oplysninger, du har indtastet.
			</h3>
			<h6>
			...
			<div class="grid">
				...
				<div class="inner-grid left-m">
					...
					<p>
						{formatNumber(
							drivingDeductionData.distanceWithDeduction.pricePrDay
						)}&nbsp;&nbsp;&nbsp;kr.
					</p>
					{#if drivingDeductionData.bridgeData.length > 1}
						<p>
				...
				{#if drivingDeductionData.bridgeData.length > 0}
					<div class="inner-grid">
						<p>
							...
							<b>{formatNumber(drivingDeductionData.deductionPrDay)}</b> kr/dag
						</p>
					</div>
				{:else}
					<div class="inner-grid">
