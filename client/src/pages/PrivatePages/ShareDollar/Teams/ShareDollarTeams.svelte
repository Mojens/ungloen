...
<script>
	...
	async function createTeam() {
		const response = await fetch(`${$BASE_URL}/api/private/sharedollar/teams`, {
			method: "POST",
			credentials: "include",
			headers: {
...
<main class="container">
	<hgroup>
		...
		<h3 class="top-m">
			Her kan du se dine egen teams, og de teams du er en del af. Du kan oprette
			nye teams, inviter andre til dit team, du kan forlade teams, og slette
			teams. I dine teams kan du sætte regnigner op og anmod om betalinger fra
			andre i dit team.
		</h3>
	</hgroup>
	<hr />
								...
								>
									<a
										...
										on:click={() => {
											...
											confirmThis(updateTeamName, team.id);
										}}
									>
									...
									<span slot="description">
										<p>Er du sikker på du vil opdatere team navnet?</p>
	...
	<div class="teams-apart-of">
		<hgroup>
			...
			<h3 class="top-m">Her forneden kan du se de teams du er en del af</h3>
		</hgroup>
		<details role="list">
								...
								{#if !ownedTeams.some((team) => team.team_creator_id === $user.id)}
									...
									>
										<a
											...
											class="icon-in-list"><i class="fa fa-user-times" /></a
										>
										<span slot="title">
											Er du sikker på, at du vil forlade dette team?
										</span>
										<span slot="description">
											Du kan ikke fortryde denne handling! <br />Du vil ikke
											længere have adgang til dette team.
											<br />Du skal kontakte team ejeren for at blive tilføjet
											igen.
										</span>
									</Confirm>
								{/if}
