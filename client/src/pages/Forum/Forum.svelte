							...
							{#if post.user_id === $user.id}
								...
								>
									...
									>
										...
										<span slot="description">
											<p>
												Efter du har opdateret indlægget, kan du ikke fortryde
											</p>
											<form>
						...
						{:else if userPostLikes.length >= 1}
							{#if userPostLikes.filter((like) => like.post_id === post.id).length > 0}
								...
								>
									...
									<span class="like-count-liked">{post.likes}</span>
								</button>
							{:else}
								<button
									class="like-button w-25"
										...
										<li class="post-item">
													...
													>
														...
														>
															<a
																class="right-m"
																on:click={() =>
																	confirmThis(deleteComment, comment.id)}
																><i class="fa fa-trash" /></a
															>
															<span slot="title">
														...
														>
															<a
																class="left-m"
																on:click={() => {
																	commentToEdit = comment.content;
																	confirmThis(updateComment, comment.id);
																}}><i class="fa fa-edit" /></a
															>
															<span slot="title"> Opdater Kommentar </span>
															<span slot="description">
																<p>
																	Efter du har opdateret din kommentar, kan du
																	ikke fortryde
																</p>
																<form>
											...
											{:else if userCommentLikes.length >= 1}
												{#if userCommentLikes.filter((like) => like.comment_id === comment.id).length > 0}
													...
													>
														<span class="like-icon">♥</span>
														<span class="like-count-liked">{comment.likes}</span
														>
													</button>
												{:else}
													<button
